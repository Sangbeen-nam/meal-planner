...
// ── 유틸 함수 ─────────────────────────────────────────────────────────────────
function pickOne(db, prefs, ingreds, usedNames = [], minAgeIndex = 3) {
  const ageSafe = db.filter(i => isSafeForAge(i, minAgeIndex) && !usedNames.includes(i.name));
  ...
  const final = pool.length > 0 ? pool : ageSafe.length > 0 ? ageSafe : db.filter(i => isSafeForAge(i, minAgeIndex));
  if (!final.length) return db[0];
  return final[Math.floor(Math.random() * final.length)];
}

function pickTwoSides(prefs, ingreds, usedNames = [], minAgeIndex = 3) {
  const ageSafeP = SIDE_DB.filter(s => s.nutriType === "단백질" && isSafeForAge(s, minAgeIndex) && !usedNames.includes(s.name));
  const ageSafeV = SIDE_DB.filter(s => s.nutriType === "채소" && isSafeForAge(s, minAgeIndex) && !usedNames.includes(s.name));
  const pFinal = ageSafeP.length > 0 ? ageSafeP : SIDE_DB.filter(s => s.nutriType === "단백질" && isSafeForAge(s, minAgeIndex));
  const vFinal = ageSafeV.length > 0 ? ageSafeV : SIDE_DB.filter(s => s.nutriType === "채소" && isSafeForAge(s, minAgeIndex));
...
function generateWeekPlan(prefs, ingreds, selectedAgeIds = []) {
  ...
  DAYS.forEach(day => {
    plan[day] = {};
    MEALS.forEach(m => {
      const rice = pickOne(RICE_DB, prefs, ingreds, usedRice, minAgeIndex);
      const soup = pickOne(SOUP_DB, prefs, ingreds, usedSoup, minAgeIndex);
      const sides = pickTwoSides(prefs, ingreds, usedSides, minAgeIndex);
...
export default function MealPlanner() {
  ...
  const handleGenerate = () => {
    setLoading(true);
    setTimeout(() => {
      setWeekPlan(generateWeekPlan(selectedFoods, selectedIngreds, selectedAges));
      setStep("planner");
      setLoading(false);
    }, 900);
  };
  ...
  const handleRegenMeal = () => {
    ...
    const rice = pickOne(RICE_DB, selectedFoods, selectedIngreds, [], minAgeIndex);
    const soup = pickOne(SOUP_DB, selectedFoods, selectedIngreds, [], minAgeIndex);
    const sides = pickTwoSides(selectedFoods, selectedIngreds, [], minAgeIndex);
    setWeekPlan(prev => {
